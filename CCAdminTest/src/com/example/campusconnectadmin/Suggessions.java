...
public class Suggessions extends ListActivity {
	...
	private void getSuggessions() {
		WebServices ws = new WebServices(getApplicationContext());

		params = new RequestParams();
		params.add("tag", "all");

		ws.invokeWebService(params, "GetSuggessions", new WSResponnse() {
			@Override
			public void onResponse(boolean success, String response) {
				if (success)
					showSuggessions(response);
			}
		}, "");
	}

	private void showSuggessions(String contactList) {
		...
		if (jArray != null) {
			for (int i = 0; i < jArray.length(); i++) {
				...
				try {
					...
					String text = null;
					text = "Student ID: "
							+ jObj.get("suggession_student_id").toString()
							+ "\n" + "Suggession Details:"
							+ jObj.get("suggession_details").toString() + "\n";
